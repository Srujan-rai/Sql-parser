INSERT INTO PRE_NCR.TB_STAGE10_3 WITH AUTO_INCREMENT SALES_DISCOUNTS_C_S_R AS
  (SELECT DISTINCT t1.COMPANY_CODE,
                   CASE
                       WHEN t1.COMPANY_CODE IN ('1020',
                                                '3000',
                                                '6000') THEN 'North East'
                       WHEN t1.COMPANY_CODE IN ('5010',
                                                '1030') THEN 'West'
  ...
  (SELECT t2.BILLINGDOCUMENT,
   ...
   LEFT JOIN SALES_DISCOUNTS_C_S_R t3 ON t2.SALES_ORGANISATION=t3.COMPANY_CODE
   AND RIGHT(CONCAT('0', t3.SP_STATE), 2)=t2.SHIP_TO_STATE
   AND t2.GEOGDISTRICTCODE=t3.RATECODE_DEPOTCODE
   AND t2.REGION=t3.REGION
   AND (CAST(t2.BILLINGDATE AS DATE) BETWEEN t3.VALID_FROM AND t3.VALID_TO)
   WHERE t2.BILLINGDATE>='20220401'
   GROUP BY t2.BILLINGDOCUMENT,
            t2.BILLINGDATE,
            t3.VALID_FROM
   UNION SELECT t4.BILLINGDOCUMENT,
                t4.BILLINGDATE,
   ...
   AND t4.GEOGDISTRICTCODE=t5.RATECODE_DEPOTCODE
   AND t4.REGION=t5.REGION
   AND (CAST(t4.BILLINGDATE AS DATE) BETWEEN t5.VALID_FROM AND t5.VALID_TO)
   WHERE t4.BILLINGDATE<'20220401'
   GROUP BY t4.BILLINGDOCUMENT,
            t4.BILLINGDATE,
            t5.VALID_FROM)
SELECT DISTINCT a.BILLINGDOCUMENT,
                a.BILLINGDATE,
...
LEFT JOIN MAN_UPLOAD.DIRECTDISPATCHDISCOINT_INV_CUS b ON a.BILLINGDOCUMENT=b.INVOICENO
AND CAST(a.BILLINGDATE AS DATE)=b.INVOICE_DATE
LEFT JOIN EDWDM.TB_PACKING c ON a.BILLINGDOCUMENT=c.BILLINGDOCUMENT
AND a.BILLINGDATE=c.BILLINGDATE
LEFT JOIN
  (SELECT SALES_ORDER_NO,
          MOTHER_PLANT,
          MAX(RATE) AS RATE
   FROM MAN_UPLOAD.DIRECTDISPATCHDISCOINT_SO_MP_CUS
   GROUP BY SALES_ORDER_NO,
            MOTHER_PLANT) d ON a.SONUMBER=d.SALES_ORDER_NO
AND a.MANUFACTURING_PLANT=d.MOTHER_PLANT
...
LEFT JOIN
  (SELECT COMPANY_CODE,
          ...
          VALID_FROM,
          VALID_TO,
          MAX(PRICE_DISCOUNT) AS PRICE_DISCOUNT
   FROM MAN_UPLOAD.SALES_PRICE_DISCOUNT_C_S_I_P_M
   GROUP BY COMPANY_CODE,
            ...
            MATERIAL_CODE,
            VALID_FROM,
            VALID_TO) g ON a.SHIP_TO_STATE=RIGHT(CONCAT('0', g.STATE_CODE), 2)
AND a.SALES_ORGANISATION=g.COMPANY_CODE
AND g.INCO_TERM=a.INCOTERMS1
AND a.PLANTCODE=g.PLANT_CODE
AND a.MATERIALNUMBER=g.MATERIAL_CODE
AND (CAST(a.BILLINGDATE AS DATE) BETWEEN g.VALID_FROM AND g.VALID_TO)
LEFT JOIN
  (SELECT COMPANY_CODE,
          SOLD_TO_PARTY_CODE,
          VALID_FROM,
          VALID_TO,
          MAX(PRICE_DISCOUNT_RATE) AS PRICE_DISCOUNT_RATE
   FROM MAN_UPLOAD.SALES_PRICE_DISCOUNT_C_SOLD
   GROUP BY COMPANY_CODE,
            SOLD_TO_PARTY_CODE,
            VALID_FROM,
            VALID_TO) h ON a.SALES_ORGANISATION=h.COMPANY_CODE
AND a.SOLDTOPARTYCODE=RIGHT(CONCAT('0000000000', h.SOLD_TO_PARTY_CODE), 10)
...
LEFT JOIN
  (SELECT FINAL_SOURCE,
   ...
   FROM EDWDM.TB_PLANT_DEPOT_DISTANCE
   GROUP BY FINAL_SOURCE,
            FINAL_DESTINATION,
            MODE_OF_SUPPLY) i ON a.MANUFACTURING_PLANT=i.FINAL_SOURCE
AND a.PLANTCODE=i.FINAL_DESTINATION
AND a.MODEOFTRANSPORT=i.MODE_OF_SUPPLY
LEFT JOIN
  (SELECT t6.COMPANY_CODE,
          t6.SOLD_TO_PARTY AS SOLD_TO_PARTY_CODE,
          MAX(t6.DISCOUNT_RATE) AS DISCOUNT_RATE,
          t6.VALID_FROM,
          t6.VALID_TO,
          t7.SHIPTOPARTYCODE
   ...
   GROUP BY t6.COMPANY_CODE,
            ...
            t6.VALID_FROM,
            t6.VALID_TO,
            t7.SHIPTOPARTYCODE) j ON j.COMPANY_CODE=a.SALES_ORGANISATION
AND a.SOLDTOPARTYCODE=j.SOLD_TO_PARTY_CODE
AND j.SHIPTOPARTYCODE=a.SHIPTOPARTYCODE
AND (CAST(a.BILLINGDATE AS DATE) BETWEEN j.VALID_FROM AND j.VALID_TO)
LEFT JOIN
  (SELECT DISTRIBUTION_CHANNEL AS DCHL,
          STATE_CODE AS SP_STATE,
          RATE_CODE AS RATECODE_DEPOT_CODE,
          VALID_FROM AS DATE_DIST_FROM,
          VALID_TO AS DATE_DIST_TO,
          MAX(NEW_DISCOUNT_4+QUARTERLY_DISCOUNT_QD) AS QUATERLY_DISCOUNT_QD_SOUTH
   FROM MAN_UPLOAD.SALES_DISCOUNT_C_DBC_S_R
   GROUP BY DISTRIBUTION_CHANNEL,
            STATE_CODE,
            RATE_CODE,
            VALID_FROM,
            VALID_TO) k ON a.DISTRIBUTIONCHANNEL=RIGHT(CONCAT('0', k.DCHL), 2)
AND a.GEOGDISTRICTCODE=k.RATECODE_DEPOT_CODE
AND a.SHIP_TO_STATE=RIGHT(CONCAT('0', k.SP_STATE), 2)
...
LEFT JOIN
  (SELECT COMPANY_CODE,
          ...
          VALID_FROM,
          VALID_TO,
          MAX(PRICE_DISCOUNT_RATE) AS PD_COUNTY_WISE
   FROM MAN_UPLOAD.SALES_PRICE_DISCOUNT_C_S_CO_M
   GROUP BY COMPANY_CODE,
            STATE_CODE,
            SH_COUNTY_CODE,
            MATERIAL_CODE,
            VALID_FROM,
            VALID_TO) l ON a.SALES_ORGANISATION=l.COMPANY_CODE
AND RIGHT(CONCAT('0', l.STATE_CODE), 2)=a.SHIP_TO_STATE
AND a.SHIP_TO_COUNTYCODE=RIGHT(CONCAT('000', SH_COUNTY_CODE), 3)
AND a.MATERIALNUMBER=l.MATERIAL_CODE
AND (CAST(a.BILLINGDATE AS DATE) BETWEEN l.VALID_FROM AND l.VALID_TO)
LEFT JOIN DISCOUNT_NEW_2 m ON a.BILLINGDOCUMENT=m.BILLINGDOCUMENT
AND a.BILLINGDATE=m.BILLINGDATE
LEFT JOIN
  (SELECT COMPANY_CODE,
          STATE_CODE,
          INCO_TERM,
          ...
          MONTHS,
          MAX(PRICE_DISCOUNT_RATE) AS PRICE_DISCOUNT_RATE
   FROM MAN_UPLOAD.SALES_PRICE_DISCOUNT_C_S_I_SD_M
   GROUP BY COMPANY_CODE,
            STATE_CODE,
            INCO_TERM,
            SALES_DISTRICT,
            MATERIAL_CODE,
            YEARS,
            MONTHS) n ON a.SALES_ORGANISATION=n.COMPANY_CODE
AND a.SHIP_TO_STATE=RIGHT(CONCAT('0', n.STATE_CODE), 2)
AND a.GEOGDISTRICTCODE=n.SALES_DISTRICT
AND a.INCOTERMS1=n.INCO_TERM
AND a.MATERIALNUMBER=n.MATERIAL_CODE
AND EXTRACT(YEAR
            FROM CAST(a.BILLINGDATE AS DATE))=n.YEARS
AND TO_CHAR(CAST(a.BILLINGDATE AS DATE), 'MONTH')=UPPER(n.MONTHS)
LEFT JOIN
  (SELECT DISTINCT UNLOADING_POINT_REGIO,
                   UNLOADING_POINT_COUNTY_CO,
                   MATERIAL_CODE,
                   MAX(PRICE_DISCOUNT_RATE) AS PRICE_DISCOUNT_RATE,
                   VALID_FROM,
                   VALID_TO
   FROM MAN_UPLOAD.SALES_PRICE_DISCOUNT_C_S_CO_M_U
   GROUP BY UNLOADING_POINT_REGIO,
            UNLOADING_POINT_COUNTY_CO,
            MATERIAL_CODE,
            VALID_FROM,
            VALID_TO) w ON RIGHT(CONCAT('0', CAST(w.UNLOADING_POINT_REGIO AS VARCHAR(6))), 2)=a.UNLOADINGPOINT_STATE
